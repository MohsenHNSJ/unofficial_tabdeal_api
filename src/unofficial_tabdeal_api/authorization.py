...
class AuthorizationClass(BaseClass):
    ...
    async def keep_authorization_key_alive(self, wait_time: int):
        """Keeps the Authorization key alive by periodically calling and using it so it won't expire
        ...
        while True:
            ...
            # First we wait, as we have already checked the authorization key at the start of program
            # This method's goal is to keep the key alive, not to check it, so use the above method instead
